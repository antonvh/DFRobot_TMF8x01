...
"""!
  ...
  @author      Arya(xue.peng@dfrobot.com) + Anton's Mindstorms
  @version     V1.0
  @date        2024-06
"""
...
class DFRobot_TMF8x01:
    ...
    def __init__(self, i2c_dev, enable_pin=15):
        """Set up i2c sensor

        Args:
            i2c_dev (I2C): i2c device to use
        """
        self.i2c = i2c_dev
    ...
    def begin(self):
        ...
        if self._get_app_id() == 0x80:
            print("Bootloader is active, patching ram")
    ...
    def is_data_ready(self):
        ...
        self.result_dict = dict(
            zip(self.result_dictKey, self._read_bytes(self.REG_MTF8x01_STATUS, 11))
        )
        if self.result_dict["regContents"] == 0x55:
            if self.result_dict["tid"] != self._tid:
                self._tid = self.result_dict["tid"]
                j = (
                    self.result_dict["syscolck3"] << 24
                    | self.result_dict["syscolck2"] << 16
                    | self.result_dict["syscolck2"] << 8
                    | self.result_dict["syscolck0"]
                )
                if self._count < 4:
    ...
    def _write_bytes(self, reg, buf):
        ...
        try:
            ...
            self.i2c.writeto_mem(self._addr, reg, bytes(buf))
            # print(reg, bytes(buf))
    ...
    def _read_bytes(self, reg, len1):
        ...
        try:
            ...
            rslt = self.i2c.readfrom_mem(self._addr, reg, len1)
            self.last_operate_status = self.STA_OK
...
class DFRobot_TMF8801(DFRobot_TMF8x01):
    def __init__(self, *args, **kwargs):
        ...
        DFRobot_TMF8x01.__init__(self, *args, **kwargs)
...
class DFRobot_TMF8701(DFRobot_TMF8x01):
    ...
    def start_measurement(self, calib_m, mode):
        ...
        if mode == self.ePROXIMITY:
            self._modify_cmd_set(
                self.CMDSET_INDEX_CMD6, self.CMDSET_BIT_PROXIMITY, True
            )
            self._modify_cmd_set(
                self.CMDSET_INDEX_CMD6, self.CMDSET_BIT_DISTANCE, False
            )
            self._modify_cmd_set(self.CMDSET_INDEX_CMD6, self.CMDSET_BIT_COMBINE, False)
        elif mode == self.eDISTANCE:
            self._modify_cmd_set(
                self.CMDSET_INDEX_CMD6, self.CMDSET_BIT_PROXIMITY, False
            )
